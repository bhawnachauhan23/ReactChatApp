export const data = {
  profile: {
    id: 1,
    name: "Bhawna Chauhan",
    image:
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSXKvXvQxHCdhX-i4dzqa4v32pGPU3JJZEpYA&usqp=CAU",
    contacts: [
      {
        id: 2,
        name: "Pooja",
        image:
          "data:image/jpeg;base64,",
        chatlog: [
          {
            text: "Hi,How are you!",
            timestamp: "10:01 AM",
            sender: "user",
            message_id: 1
          },
          {
            text: "I am fine. How are you!",
            timestamp: "10:02 AM",
            sender: "me",
            message_id: 2
          },
          {
            text: "I am great.",
            timestamp: "10:03 AM",
            sender: "user",
            message_id: 3
          },
          {
            text:
              "Or maybe not, let me check something and call you. Give me sometime",
            timestamp: "10:04 AM",
            sender: "user",
            message_id: 4
          },
          {
            text: "Oky dear !",
            timestamp: "10:05 AM",
            sender: "me",
            message_id: 5
          }
        ]
      },
      {
        id: 3,
        name: "Anup Pal",
        image:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRYnYxRcqboNmFxjq90oByETdRoJBnMLypAuA&usqp=CAU",
        chatlog: [
          {
            text: "Hi Dear,How are you!",
            timestamp: "10:01 AM",
            sender: "user",
            message_id: 1
          },
          {
            text: "I am good. How are you!",
            timestamp: "10:02 AM",
            sender: "me",
            message_id: 2
          },
          {
            text: "I am fine.",
            timestamp: "10:03 AM",
            sender: "user",
            message_id: 3
          },
          {
            text: "What are you doing?",
            timestamp: "10:04 AM",
            sender: "user",
            message_id: 4
          },
          {
            text: "Nothing..Your birthday is coming..What u want?",
            timestamp: "10:05 AM",
            sender: "me",
            message_id: 5
          }
        ]
      },
      {
        id: 4,
        name: "Prem",
        image:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcThKK1yPSJ5VT0NIh5Q7LSRoZGEccBw0Ni0_A&usqp=CAU",
        chatlog: [
          {
            text: "Hey! Whats up..!!",
            timestamp: "10:01 PM",
            sender: "user",
            message_id: 1
          },
          {
            text: "Watching IPL",
            timestamp: "10:02 PM",
            sender: "me",
            message_id: 2
          },
          {
            text: "Great.Whats the score?",
            timestamp: "10:30 PM",
            sender: "user",
            message_id: 3
          },
          {
            text: "Chennai is about to win",
            timestamp: "11:00 PM",
            sender: "user",
            message_id: 4
          },
          {
            text: "Yeahhhhh!",
            timestamp: "11:05 PM",
            sender: "me",
            message_id: 5
          }
        ]
      },
      {
        id: 5,
        name: "Vandana",
        image:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQfR9o9Pbt3znQnWr2VyrGe5bCC2p3UjMEwpg&usqp=CAU",
        chatlog: []
      },
      {
        id: 6,
        name: "Shreya",
        image:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTFns-rvzQJdg1L0OU7Rmzp1PG_Q4pyMBoupw&usqp=CAU",
        chatlog: [
          {
            text: "Hey Bhawna, why you are not recieving my calls?",
            timestamp: "10:01 AM",
            sender: "user",
            message_id: 1
          },
          {
            text: "I was bit busy",
            timestamp: "10:02 AM",
            sender: "me",
            message_id: 2
          },
          {
            text: "Are you sure,  Bhawna.",
            timestamp: "10:03 AM",
            sender: "user",
            message_id: 3
          },
          {
            text: "Is there any problem??",
            timestamp: "10:04 AM",
            sender: "user",
            message_id: 4
          },
          {
            text: "I am absolutely fine",
            timestamp: "10:05 AM",
            sender: "me",
            message_id: 5
          }
        ]
      },
      {
        id: 7,
        name: "Aishwarya",
        image:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRBwgu1A5zgPSvfE83nurkuzNEoXs9DMNr8Ww&usqp=CAU",
        chatlog: [
          {
            text: "Hey dear, Arun is asking for your number.",
            timestamp: "10:06 AM",
            sender: "me",
            message_id: 1
          },
          {
            text: "Did you tell him anything?.",
            timestamp: "10:06 AM",
            sender: "user",
            message_id: 2
          },
          {
            text: "No, I haven't gave him yet!!!.",
            timestamp: "10:07 AM",
            sender: "me",
            message_id: 3
          },
          {
            text: "What do you mean yet!!!.",
            timestamp: "10:08 AM",
            sender: "user",
            message_id: 4
          },
          {
            text:
              "Nothing dear, You take care now and if you need any help just tell me",
            timestamp: "10:09 AM",
            sender: "me",
            message_id: 5
          },
          {
            text: "Sure, Take care !",
            timestamp: "10:09 AM",
            sender: "user",
            message_id: 6
          }
        ]
      },
      {
        id: 8,
        name: "Mouni Roy",
        image:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQhf1wxZAl8SDAgUG32w6RasqVRF-ggOC52IQ&usqp=CAU",
        chatlog: []
      },
      {
        id: 9,
        name: "Naksh",
        image:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcStpnvIjddYVfoHdp--SEOqUb21eFOUKuIk3g&usqp=CAU",
        chatlog: []
      }
    ]
  }
};
